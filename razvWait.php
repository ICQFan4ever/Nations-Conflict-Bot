...
if(isset($_GET['force']))
	...
	{
			$query = 'SELECT * FROM `razv` WHERE `acc_id` = '.$_ACC['id'].' AND `wait` = \'0\' AND `guard` =\'0\' ORDER BY `id` ASC';
	}
...
while($razv = mysql_fetch_assoc($q))
	{
		$ch = curl_safe_init('http://nations.mgates.ru/conflict/game.php?q=control&id_unit='.$razv['id_unit'].'&action_id=4');
		// прокси
		...
		if(preg_match("#Такой боевой единицы нет#u", $a))
			...
			{
				mysql_query("UPDATE `razv` SET `wait` = '1' WHERE `id` = ".$razv['id']);
			}
	}